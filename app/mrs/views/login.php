<!DOCTYPE html>
<html lang="zh">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SaaS 登录 - MRS 系统</title>
    <style>
/* 路径: css/login-v2.css */

:root {
    --primary-color: #00A389; /* 深青色 */
    --accent-color: #12C2E9; /* 亮蓝 */
    --text-color: #333;
    --panel-bg: #1A202C; /* 深色背景 */
    --card-bg-color: #FFFFFF;
}

body {
    font-family: 'Inter', 'Roboto', 'PingFang SC', sans-serif;
    margin: 0;
    min-height: 100vh;
    display: flex;
}

.split-container {
    display: flex;
    width: 100vw;
    min-height: 100vh;
}

/* --- 左侧宣传区 --- */
.marketing-panel {
    flex: 1;
    background: linear-gradient(135deg, var(--panel-bg), #2C3E50);
    color: white;
    display: flex;
    justify-content: center;
    align-items: center;
    padding: 60px;
    position: relative;
    overflow: hidden; /* 隐藏溢出内容 */
}

/* 抽象图形效果（可选，提升科技感） */
.marketing-panel::before {
    content: '';
    position: absolute;
    top: -10%;
    left: -10%;
    width: 120%;
    height: 120%;
    background: radial-gradient(circle at 10% 90%, rgba(0, 163, 137, 0.4) 0%, transparent 50%);
    animation: pulse 10s infinite alternate;
}

@keyframes pulse {
    from { transform: scale(1); }
    to { transform: scale(1.1); }
}

.marketing-content {
    max-width: 450px;
    z-index: 10;
}

.marketing-content h1 {
    font-size: 2.5rem;
    font-weight: 700;
    margin-bottom: 20px;
    border-left: 4px solid var(--accent-color);
    padding-left: 15px;
}

.marketing-content p {
    font-size: 1.1rem;
    line-height: 1.6;
    margin-bottom: 30px;
}

.feature-list div {
    font-size: 1rem;
    margin-bottom: 12px;
}

.feature-list i {
    color: var(--accent-color);
    margin-right: 10px;
}


/* --- 右侧登录区 --- */
.login-panel {
    flex: 1;
    display: flex;
    justify-content: center;
    align-items: center;
    background-color: var(--card-bg-color);
}

.login-card {
    width: 100%;
    max-width: 400px;
    padding: 30px;
}

.logo-top {
    font-size: 1.5rem;
    font-weight: 700;
    color: var(--primary-color);
    margin-bottom: 40px;
}

h2 {
    font-size: 1.8rem;
    margin-bottom: 5px;
    font-weight: 700;
    color: var(--text-color);
}

.subtitle {
    color: #666;
    margin-bottom: 30px;
    font-size: 0.95rem;
}

/* 错误提示 */
.alert {
    padding: 12px 15px;
    margin-bottom: 20px;
    border-radius: 4px;
    background-color: #fee;
    border: 1px solid #fcc;
    color: #c33;
    font-size: 0.9rem;
}

/* 浮动标签输入框 (Floating Labels) */
.input-group {
    position: relative;
    margin-bottom: 25px;
}

.input-group input {
    width: 100%;
    padding: 15px 10px;
    border: 1px solid #CCC;
    border-radius: 4px;
    font-size: 1rem;
    box-sizing: border-box;
    transition: border-color 0.3s;
}

.input-group label {
    position: absolute;
    left: 10px;
    top: 50%;
    transform: translateY(-50%);
    color: #999;
    pointer-events: none;
    transition: all 0.2s ease-out;
    background: white;
    padding: 0 5px;
}

.input-group input:focus,
.input-group input:not(:placeholder-shown) {
    border-color: var(--primary-color);
    outline: none;
}

.input-group input:focus + label,
.input-group input:not(:placeholder-shown) + label {
    top: 0;
    transform: translateY(-50%) scale(0.9);
    font-size: 0.8rem;
    color: var(--primary-color);
}

.options {
    text-align: right;
    margin-bottom: 25px;
}

.forgot-password {
    color: var(--primary-color);
    font-size: 0.9rem;
    text-decoration: none;
}

.login-button {
    width: 100%;
    padding: 14px;
    background-color: var(--primary-color);
    color: white;
    border: none;
    border-radius: 4px;
    font-size: 1rem;
    font-weight: 600;
    cursor: pointer;
    transition: background-color 0.3s;
    margin-bottom: 20px;
}

.login-button:hover {
    background-color: #007A65;
}

.divider {
    text-align: center;
    margin: 25px 0;
    position: relative;
    color: #AAA;
}

.divider::before,
.divider::after {
    content: '';
    position: absolute;
    top: 50%;
    width: 40%;
    height: 1px;
    background: #EEE;
}

.divider::before { left: 0; }
.divider::after { right: 0; }

.divider span {
    background: white;
    padding: 0 10px;
    font-size: 0.9rem;
}

.social-button {
    width: 100%;
    padding: 12px;
    font-size: 1rem;
    border: 1px solid #DDD;
    background: white;
    color: #555;
    border-radius: 4px;
    cursor: pointer;
    transition: border-color 0.3s;
}

.social-button:hover {
    border-color: var(--primary-color);
    color: var(--primary-color);
}

.social-button i {
    margin-right: 10px;
}

.register-link {
    margin-top: 30px;
    text-align: center;
    font-size: 0.9rem;
}

.register-link a {
    color: var(--primary-color);
    text-decoration: none;
}

/* 媒体查询：在小屏幕上堆叠 */
@media (max-width: 768px) {
    .split-container {
        flex-direction: column;
    }

    .marketing-panel {
        display: none; /* 在手机上隐藏宣传区 */
    }

    .login-panel {
        min-height: 100vh;
    }
}
    </style>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
</head>
<body>
    <div class="split-container">
        <div class="marketing-panel">
            <div class="marketing-content">
                <h1>提升您的工作效率</h1>
                <p>一个平台，管理您的所有数据和协作流程。安全、快速、可靠。</p>
                <div class="feature-list">
                    <div><i class="fas fa-chart-line"></i> 实时数据分析</div>
                    <div><i class="fas fa-users"></i> 无缝团队协作</div>
                    <div><i class="fas fa-lock"></i> 顶级安全保障</div>
                </div>
            </div>
        </div>

        <div class="login-panel">
            <div class="login-card">
                <div class="logo-top">MRS</div>
                <h2>欢迎登录</h2>
                <p class="subtitle">使用您的账户访问系统。</p>

                <?php if (isset($_GET['error'])): ?>
                    <div class="alert">
                        用户名或密码错误。
                    </div>
                <?php endif; ?>

                <form action="/mrs/be/index.php?action=process_login" method="post">
                    <div class="input-group">
                        <input type="text" id="username" name="username" placeholder=" " required autofocus>
                        <label for="username">登录账号</label>
                    </div>

                    <div class="input-group">
                        <input type="password" id="password" name="password" placeholder=" " required>
                        <label for="password">密码</label>
                    </div>

                    <div class="options">

                    </div>

                    <button type="submit" class="login-button">
                        登录系统
                    </button>

                    <div class="divider">
                        <span>或使用</span>
                    </div>

                    <button type="button" class="social-button google-btn">
                        <i class="fa-solid fa-circle-info"></i> 没其他办法了
                    </button>
                </form>

                <div class="register-link"></div>
            </div>
        </div>
    </div>
</body>
</html>
