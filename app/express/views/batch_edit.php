...
<body>
    ...
    <script>
        document.getElementById('batch-edit-form').addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('/express/exp/index.php?action=batch_update', {
                    method: 'POST',
                    headers: {
                ...
                if (data.success) {
                    ...
                    messageDiv.textContent = '保存成功，正在返回详情...';
                    messageDiv.style.display = 'block';

                    setTimeout(() => {
                        window.location.href = '/express/exp/index.php?action=batch_detail&batch_id=' + payload.batch_id;
                    }, 800);
                } else {
                    messageDiv.className = 'message error';
