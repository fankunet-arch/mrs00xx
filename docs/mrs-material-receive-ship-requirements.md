...
- 所有换算与库存最终形态均以“标准单位整数”为基准：  
  - 箱数/包数在中间计算过程中可以出现小数，但系统在保存最终结果前必须先折算为标准单位整数，再拆回“整箱 + 散件”。  
  - 例如规则为 1 箱 = 10 瓶时，6.5 箱一律视为 65 瓶，再拆解为 6 箱 + 5 瓶；系统不允许以“6.5 箱”这种形式直接作为最终库存记账单位。
