...
**功能说明**: 仓库清点功能用于定期或不定期盘点仓库货物，确认实物与系统记录的一致性。支持手机端操作，可分多天完成清点任务。

**访问路径**: `/mrs/index.php` (前台入口)
...
1. **访问清点首页**
   - 使用手机浏览器访问: `https://your-domain.com/mrs/index.php`
   - 进入MRS仓库清点系统

2. **创建清点任务**
   - 点击「开始新清点」按钮
   - 填写清点任务名称（例如: "2025年1月仓库盘点"）
   - 可选填写创建人姓名和备注
   - 点击「开始清点」
...
**提示**:
- 清点任务创建后状态为"进行中"，可以随时暂停和继续
- 支持多天分批完成清点工作
- 建议为每次清点任务命名明确，便于后续查询

#### 4.9.2 箱级清点（快速模式）

**适用场景**: 只需确认箱子是否在仓库中，不需要打开箱子核对数量。
...
```
仓库员小张拿着手机在仓库走动
 → 看到箱号A001 → 输入"A001" → 找到 → 选"只确认箱子" → 保存
 → 看到箱号A002 → 输入"A002" → 找到 → 选"只确认箱子" → 保存
 → ... 继续清点
```

#### 4.9.3 数量级清点（详细模式）

**适用场景**: 需要打开箱子，核对箱内实际数量是否与系统记录一致。
...
打开箱号B001
 → 箱内有: 红酒12瓶、杯子24个
 → 点击"核对数量"
 → 添加物品1: SKU名称"红酒", 系统数量12, 实际数量12
 → 添加物品2: SKU名称"杯子", 系统数量20, 实际数量24 (发现多了4个!)
 → 保存(系统自动标记为"数量有差异")
```

#### 4.9.4 处理系统中不存在的箱子

**场景**: 仓库中发现箱号X999，但系统中查不到。
...
3. **查看清点报告**
   ...
   **未清点箱子列表**:
   - 系统中有但未被清点的箱号
   - 例如: BAT0050, BAT0051, BAT0055...

   **差异明细**:
   - 数量有差异的箱子详细信息
   - 例如: B001(红酒: 系统12实际12, 杯子: 系统20实际24, 差异+4)
...
【系统中有但未清点】(共20箱)
BAT0050, BAT0051, BAT0055, ...

【数量有差异】(共5箱)
1. B001: 杯子(系统20, 实际24, +4)
2. B005: 红酒(系统12, 实际10, -2)
...
...
**效率技巧**:
1. **按区域清点**: 按仓库区域顺序清点，避免遗漏
2. **箱级优先**: 大部分使用箱级清点，仅重点箱子用数量级
3. **及时保存**: 每清点一箱立即保存，避免数据丢失
4. **分批完成**: 大仓库可分多天完成，不必一次性清完

**注意事项**:
- ✅ 手机操作需要稳定网络连接
- ✅ 输入框字体较大，方便手机输入
- ✅ 清点记录可追溯，包含操作时间和操作人
- ✅ 完成清点后任务状态变为"已完成"，无法再添加记录
- ⚠️ 不支持扫描功能（简化实现）
- ⚠️ 不支持货架号管理
- ⚠️ 未引入箱规概念
...
Q: 清点过程中手机断网了怎么办？
A: 重新连接网络后，继续清点即可。已保存的记录不会丢失。

Q: 误清点了错误的箱子怎么办？
A: 清点记录目前无法删除，可在完成报告中查看并人工核对。

Q: 清点任务可以删除吗？
A: 已完成的清点任务不能删除，但可以创建新任务重新清点。

Q: 差异记录如何处理？
A: 根据实际情况，可在后台进行库存调整，但不在清点模块中处理。
